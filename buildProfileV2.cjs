...
let fbCode = fs.readFileSync('src/lib/firebase.js', 'utf8');
if (!fbCode.includes('getStorage')) {
  fbCode = fbCode.replace(/import \{ getAuth \} from "firebase\/auth";/, 'import { getAuth } from "firebase/auth";\nimport { getStorage } from "firebase/storage";');
  fbCode += '\nexport const storage = getStorage(app);\n';
  fs.writeFileSync('src/lib/firebase.js', fbCode, 'utf8');
}

// 2. Prepare App.jsx updates
...
// Ensure Firebase Storage imports in App
if (!appCode.includes('ref,')) {
  appCode = appCode.replace(/export const AppContext/, "import { storage } from './lib/firebase';\nimport { ref, uploadBytes, getDownloadURL } from 'firebase/storage';\n\nexport const AppContext");
}

// Remove the old ProfileTab component using a simple split strategy
const splitToken = 'function ProfileTab() {';
if (appCode.includes(splitToken)) {
  const parts = appCode.split(splitToken);
  // Re-split using export default function App to find where ProfileTab ends
  const parts2 = parts[1].split('export default function App() {');
  appCode = parts[0] + '\n' + 'export default function App() {' + parts2.slice(1).join('export default function App() {');
}
                ...
                <div key={exp.id} className="bg-white border border-purple-50 rounded-2xl p-4 shadow-sm">
                   ...
                   <div className="mt-4 flex items-center justify-between border-t border-gray-50 pt-3">
                      ...
                      <div className="flex items-center space-x-2">
                         ...
                         <button onClick={() => handleAction(exp.id, 'reimbursed')} className="px-4 h-8 flex items-center justify-center bg-green-50 text-green-600 font-bold text-xs rounded-lg hover:bg-green-100 transition">
                           <Check size={14} className="mr-1" strokeWidth={3} /> Settle
                         </button>
                      </div>
                   </div>
                </div>
                      ...
                      teamReimbursed.map(exp => (
                        <div key={exp.id} className="bg-white border border-gray-100 rounded-xl p-3 flex justify-between">
                           ...
                           <div className="text-right">
                              ...
                              {exp.receiptUrl && (
                                <button onClick={() => setSelectedBill(exp.receiptUrl)} className="text-[10px] text-blue-500 flex items-center justify-end hover:underline mt-1">
                                  <ExternalLink size={10} className="mr-1"/> View Bill
                                </button>
                              )}
                           </div>
                      ...
                      myReimbursed.map(exp => (
                        <div key={exp.id} className="bg-white border border-green-50 rounded-xl p-4 flex justify-between shadow-sm">
                           ...
                           <div className="text-right">
                              ...
                              {exp.receiptUrl && (
                                <button onClick={() => setSelectedBill(exp.receiptUrl)} className="text-[10px] text-blue-500 flex flex-center hover:underline mt-1">
                                  <ExternalLink size={10} className="mr-1"/> View Bill
                                </button>
                              )}
                           </div>
...
console.log('High Fidelity Profile Tab wired and injected successfully!');
