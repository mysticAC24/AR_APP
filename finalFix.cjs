...
// 2. Add Firebase Storage and context imports
...
// 3. Mount the Profile Tab in the Navigation
const activeTabHome = \`{activeTab === 'home' && <HomeTab />}\`;
...
const leaderboardNav = \`<NavButton icon={<Trophy size={24} />} label="Rankings" isActive={activeTab === 'leaderboard'} onClick={() => setActiveTab('leaderboard')} />\`;
...
// 4. Fix Latency & Anonymous Login missing profile bug
const legacyLoad = \`
  useEffect(() => {
...
const newLoad = \`
  import { signInAnonymously, onAuthStateChanged } from 'firebase/auth'; // Safe as top level is simulated handled below
...
// Actually we need to ensure the import is top level.
if (!code.includes('signInAnonymously')) {
  code = \`import { signInAnonymously, onAuthStateChanged } from 'firebase/auth';\\n\` + code;
}

// remove the latency block
code = code.replace(legacyLoad, \`  // Fixed load block\\n  useEffect(() => {
    if (!isLoggedIn) return;
    const unsubUsers = onSnapshot(collection(db, 'users'), (snap) => {
      const fetchedUsers = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      setUsers(fetchedUsers);
...
code = code.replace(/<LoginPage onLogin={\(userData\) => {/, \`<LoginPage onLogin={async (userData) => {
      if (!auth.currentUser) {
...
// Delete AppContext inline export to fix Fast Refresh
code = code.replace(/export const AppContext = createContext\\(\\);\\n/g, '');


// 5. Append ProfileTab V3 exactly as requested
const profileV3 = \`
    ...
    <div className="flex flex-col h-full animate-in fade-in pb-20 bg-gray-50 overflow-y-auto w-full">
      ...
      <div className="bg-white pb-6 shadow-sm relative rounded-b-3xl">
         ...
         <div className="relative h-40 w-full bg-gray-200">
            ...
            <div className="absolute top-4 right-4 bg-white/50 backdrop-blur-md p-2 rounded-full cursor-pointer hover:bg-white/80 transition shadow-sm z-10 w-9 h-9 flex items-center justify-center">
         ...
         <div className="px-6 relative -mt-16 text-left">
            <div className="relative inline-block">
               ...
               <div className="absolute bottom-2 right-2 bg-white p-2 rounded-full shadow-md cursor-pointer border border-gray-100 hover:bg-gray-50 z-10 flex items-center justify-center w-8 h-8">
            ...
            <div className="mt-2 flex justify-between items-start">
               <div>
                 <h1 className="text-2xl font-black text-gray-900 flex items-center">
                    {profileData.fullName || profileData.name} <CheckCircle2 size={18} className="text-blue-500 ml-2" />
                 </h1>
      ...
      <div className="px-5 mt-6 space-y-7">
        ...
        {isTeamLeader ? (
          ...
          <div className="space-y-4">
             ...
             <div className="bg-white/60 border border-green-100 rounded-2xl overflow-hidden mt-6 mb-10 shadow-sm">
                      ...
                      myReimbursed.map(exp => (
                        <div key={exp.id} className="bg-white border border-green-50 rounded-xl p-4 flex justify-between shadow-sm">
                           ...
                           <div className="text-right">
                              <div className="font-black text-gray-800 text-lg">₹{exp.amount}</div>
                              {exp.receiptUrl && (
                                <button onClick={() => setSelectedBill(exp.receiptUrl)} className="text-[10px] text-blue-500 flex flex-center hover:underline mt-1 justify-end">
                                  <ExternalLink size={10} className="mr-1"/> View Bill
                                </button>
                              )}
                           </div>
...
if (!code.includes('function ProfileTab() {')) {
  code = code + '\\n' + profileV3;
}
